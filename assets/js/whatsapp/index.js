...
$whatsappWebView.attr("preload", "../../js/whatsapp/preload.js");
...
whatsappWebView.addEventListener("dom-ready", function (event) {
    ...
    fetch("../../js/whatsapp/inject/WaitAuth.js").then(res => res.text()).then(WaitAuth => {
...
whatsappWebView.addEventListener('ipc-message', (event) => {
    ...
    switch (event.channel) {
        case "authenticated": {
            fetch("../../js/whatsapp/inject/ModuleRaid.js").then(res => res.text()).then(ModuleRaid => {
                whatsappWebView.executeJavaScript(ModuleRaid);

                fetch("../../js/whatsapp/inject/ExposeStore.js").then(res => res.text()).then(ExposeStore => {
                    whatsappWebView.executeJavaScript(ExposeStore);

                    fetch("../../js/whatsapp/inject/Utils.js").then(res => res.text()).then(Utils => {
                        whatsappWebView.executeJavaScript(Utils);

                        fetch("../../js/whatsapp/inject/Functions.js").then(res => res.text()).then(Functions => {
        ...
        case "onMessage": {
            ...
            setTimeout(() => delete message_times[number], 2 * 60 * 60 * 1000); // 2h
        }
    }
})
...
function SendOrderStatusMessage(order) {
    ...
    whatsapp.sendMessage(order.phone_client, message);
}