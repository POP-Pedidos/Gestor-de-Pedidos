...
function ChangeTab(for_panel) {
	...
	if (for_panel === "dashboard") {
		...
		// internal.whatsapp_visible = true;
		// $(".page-wrapper").removeClass("pinned");
		// $(".page-wrapper .sidebar-wrapper").addClass("no-transition");
...
function LoadOrderOnElement($element, order, actions = true) {
	...
	$print_options = $(`<div class="order-print-options">
		<div>
			...
			<main>
				<div value="control">
					<img src="../../img/box-analytics.png" />
					<span>Controle</span>
				</div>
				<div value="production">
					<img src="../../img/box-todo.png" />
					<span>Produção</span>
				</div>
				<div value="delivery">
					<img src="../../img/box-delivery.png" />
					<span>Entrega</span>
				</div>
			</main>
		</div>
	...
	$print_options.find("main>div").on("click", function () {
		...
		if (value === "control") {
			internal.printControlCopy(JSON.stringify(primary_printer_device), JSON.stringify(order_data), JSON.stringify(company));
		} else if (value === "delivery") {
			internal.printDeliveryCopy(JSON.stringify(primary_printer_device), JSON.stringify(order_data), JSON.stringify(company));
			...
			for (const id_printer in printer_items) {
				...
				internal.printProductionCopy(JSON.stringify(printer), JSON.stringify(order_data), JSON.stringify(company));
			}
		}
