var datatable_categorias = $("#table-cadastros-categorias").DataTable({
    language: { url: "../../js/dataTables.lang.json" },
    ordering: false,
    searching: false,
    paging: false,
    info: false,
...
$("#btnModalCategories").on("click", function (e) {
    ...
    $("#frmCategory").find(".form-image-selector>img").attr("src", "../../../img/image-upload.png").removeClass("selected");
...
$("#frmCategory").on("submit", function (e) {
    ...
    FetchAPI(`/category`, { method: "POST", body: formData }).then(category_data => {
        ...
        const rowNode = datatable_categorias.row.add([
            `<img src="${category_data.image || `${api_url}/static/img/no-image.svg`}" width="32" />`,
            category_data.name,
...
function EditCategory(id) {
    ...
    $("#frmEditCategory").find(".form-image-selector>img").attr("src", "../../../img/image-upload.png").removeClass("selected");
...
lazy_loading.onHandle = async (state) => {
    ...
    FetchAPI(`/company/${company.id_company}/categories`, {
        ...
        for (const category of categories_data.results || []) {
            ...
            const rowNode = datatable_categorias.row.add([
                `<img src="${category.image || `${api_url}/static/img/no-image.svg`}" width="32" />`,
                category.name,
