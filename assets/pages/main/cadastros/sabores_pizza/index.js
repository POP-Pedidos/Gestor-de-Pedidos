...
function addComplement(complement) {
    ...
    $complement.find(".input-cod-pdv").on("change", function () {
        ...
        FetchAPI(`/company/${company.id_company}/flavors`, {
            body: { name: complement.name, data: {"sku": codPDV} },
            method: "PUT",
        }).then(() => {
            ...
            Swal.fire("Opss...", `Ocorreu um erro ao tentar atualizar o codigo PDV do sabor ${complement.name}!`, "error");
    ...
    $complement.find(">header>.right>.switch_enable>input").on("change", function () {
        ...
        FetchAPI(`/company/${company.id_company}/flavors`, {
            body: { name: complement.name, data: {enabled: enabled} },
            method: "PUT",
        }).then(() => {
            ...
            Swal.fire("Opss...", `Ocorreu um erro ao tentar ${enabled ? "ativar" : "pausar"} o sabor ${complement.name}!`, "error");
            Swal.showValidationMessage(errorMessage);
        }).finally(() => {
    ...
    $complement.find(">footer .load-more").click(function () {
        ...
        FetchAPI(`/company/${company.id_company}/product`, {
            instance_check: true,
            params: { pizza_flavor: complement.name, offset, limit: page_limit }
        }).then(products => {
...
lazy_loading.onHandle = async (state) => {
    ...
    FetchAPI(`/company/${company.id_company}/flavors`, {
        ...
        state.max = complement_data.metadata?.max || 1;

        if(complement_data.results) for (const complement of complement_data.results) addComplement(complement);
        else {addComplement(complement_data.data)};
    }).catch(errorMessage => {
        console.error(errorMessage);
        Swal.fire("Opss...", `Ocorreu um erro ao tentar listar os sabores!`, "error");
        Swal.showValidationMessage(errorMessage);
    }).finally(() => {
