    ...
    if (!!products_data?.results.length) {
        ...
        for (const product of products_data.results) {
            ...
            const $product = $(`<div class="top-product">
                <img src="https://api.poppedidos.com.br/static/img/no-image.svg" class="avatar" />
                <div class="agent-details">
                    <h6>Erro</h6>
        ...
        $(".top-products-container").html(`<img src="../../img/empty-folder.svg"/>`);
    }
}
    ...
    if (!!client_data?.results.length) {
        ...
        for (const client of client_data?.results || []) {
            ...
            GetGenderByName(client.name_client).then(gender => {
                ...
                $img.attr("src", `../../img/${images[randomInt(0, images.length - 1)]}.svg`);
        ...
        $(".dashboard-results-container .top-clients-container").html(`<img src="../../img/empty-folder2.svg"/>`);
    }
}
...
lazy_loading.onHandle = (state) => {
    ...
    state.request.then(orders_data => {
        ...
        if (state.max === 0) $(".dashboard-results-container .orders>main").html(`<img src="../../img/order.svg"/>`);
    }).catch(error => {
        $(".dashboard-results-container .orders>main").html(`<img src="../../img/order.svg"/>`);
